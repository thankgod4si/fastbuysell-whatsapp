...
function NavLink({ item, active }: { item: Pick<NavItem, 'href' | 'label' | 'icon' | 'color'>; active: boolean }) {
  return (
    <Link
      href={item.href}
    ...
    >
      ...
      {active && <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: item.color }} />}
    </Link>
  )
}
...
function SectionLabel({ children }: { children: string }) {
  return (
    <p className="text-[#8E8E93] text-[10px] font-semibold uppercase tracking-widest px-3 mb-1">
      {children}
    </p>
  )
}
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {banner && (
        <Link
          href={status === 'pending_approval' ? '/pending-approval' : '/subscribe'}
          className="mx-3 mt-3 flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl transition-opacity hover:opacity-80"
          style={{
        ...
        >
          ...
          <div>
            <p className="text-xs font-semibold leading-tight" style={{ color: banner.color }}>
              {banner.label}
            </p>
            {banner.sub && profile && (
              <p className="text-[10px] mt-0.5" style={{ color: `${banner.color}99` }}>
                {banner.sub(profile)}
              </p>
            )}
          </div>
      ...
      <div className="p-3 border-t border-black/[0.05]">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-2xl bg-black/[0.03]">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[#1C1C1E] text-xs font-semibold truncate">
